...
namespace Account.Domain.UseCases.Write.Withdraw
{
    public sealed class WithdrawUseCase
        : IRequestHandler<WithdrawCommand, WithdrawOutput>
    {
        ...
        public async Task<WithdrawOutput> Handle(
            WithdrawCommand command,
            CancellationToken token)
        {
            var validationResult = await new WithdrawValidation()
                .ValidateAsync(
                    command,
                    token);
            ...
            account
                .Withdraw((decimal)command.Amount);
            await _write
                .UnitOfWork 
